...
const CreatePost = () => {
  ...
  // Passing prompt to backend to get a response
  const generateImage = async () => {
    if (form.prompt) {
      try {
        setGeneratingImg(true);
        const response = await fetch("https://ai-image-social.onrender.com/api/v1/image", {
          method: "POST",
          headers: {
  ...
  const handleSubmit = async (e) => {
    ...
    if (form.prompt && form.photo) {
      ...
      try {
        const response = await fetch("https://ai-image-social.onrender.com/api/v1/post", {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
          },
